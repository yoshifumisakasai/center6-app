...
const Firebase_state_level1 = (props: any) => {

    let navigate = useNavigate();
    let goBack = () => {
        navigate(-1);
    };
    const [state_level_1, set_State_level_1_Form] = useState<any>([]);
    ...
    const check_level_1 = (event: any) => {
        ...
        //下記値は、インデックス番号
        if (state_level_1.includes(event.target.value)) {
            set_State_level_1_Form(state_level_1.filter((checkedValue: any) => checkedValue !== event.target.value));
            // console.log('[Include]チェックボックスChecked個数', level_1);
        } else {
            state_level_1.push(event.target.value);
            set_State_level_1_Form(state_level_1);
        ...
        props.handleValueChange1(state_level_1);
...
export default Firebase_state_level1;