...
const Firebase_css_level3 = (props: any) => {
    ...
    const check_level_1 = (event: any) => {
        ...
        //下記値は、インデックス番号
        if (css_level_3.includes(event.target.value)) {
            set_css_level_3_Form(css_level_3.filter((checkedValue: any) => checkedValue !== event.target.value));
            // console.log('[Include]チェックボックスChecked個数', level_1);
        } else {
            css_level_3.push(event.target.value);
            set_css_level_3_Form(css_level_3);
        ...
        props.handleValueChange1(css_level_3);
    ...
    useEffect(() => {
        ...
        const usersCollectionRef1 = doc(db, 'front_library', 'level_3');
...
export default Firebase_css_level3;