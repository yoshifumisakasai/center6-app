...
function Kanri_Firebase_framework_level3() {
    ...
    const handleSubmit = async (event: any) => {
        ...
        const userDocumentRef1 = doc(db, 'front_framework', 'level_3');
        if (input1 === "") {
        } else {
    ...
    const changeUser = async (name: any) => {
        ...
        const ref = doc(db, 'front_framework', selectedOption);
        if (level_1 === "") {
        } else {
            for (let i = 0; i < level_1.length; i++) {
                await updateDoc(ref,
...
export default Kanri_Firebase_framework_level3;