...
const Firebase_db_level1 = (props: any) => {
    ...
    const [DB_level_1, set_DB_level_1_Form] = useState<any>([]);
    ...
    const check_level_1 = (event: any) => {
        ...
        //下記値は、インデックス番号
        if (DB_level_1.includes(event.target.value)) {
            set_DB_level_1_Form(DB_level_1.filter((checkedValue: any) => checkedValue !== event.target.value));
            // console.log('[Include]チェックボックスChecked個数', level_1);
        } else {
            DB_level_1.push(event.target.value);
            set_DB_level_1_Form(DB_level_1);
        ...
        props.handleValueChange1(DB_level_1);
