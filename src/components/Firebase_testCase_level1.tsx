...
const Firebase_testCase_level1 = (props: any) => {

    let navigate = useNavigate();
    let goBack = () => {
        navigate(-1);
    };
    const [testCase_level_1, set_testCase_level_1_Form] = useState<any>([]);
    ...
    const check_level_1 = (event: any) => {
        ...
        //下記値は、インデックス番号
        if (testCase_level_1.includes(event.target.value)) {
            set_testCase_level_1_Form(testCase_level_1.filter((checkedValue: any) => checkedValue !== event.target.value));
            // console.log('[Include]チェックボックスChecked個数', level_1);
        } else {
            testCase_level_1.push(event.target.value);
            set_testCase_level_1_Form(testCase_level_1);
        ...
        props.handleValueChange1(testCase_level_1);
    ...
    // };
    const entity: HomeEntity = {
        testCase_level_1: testCase_level_1
    };
    ...
    useEffect(() => {
        ...
        const usersCollectionRef1 = doc(db, 'front_testCase', 'level_1');
...
export default Firebase_testCase_level1;