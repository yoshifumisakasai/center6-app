...
const Firebase_hook_level3 = (props: any) => {

    let navigate = useNavigate();
    let goBack = () => {
        navigate(-1);
    };
    const [Hook_level_3, set_Hook_level_3_Form] = useState<any>([]);
    ...
    const check_level_1 = (event: any) => {
        ...
        //下記値は、インデックス番号
        if (Hook_level_3.includes(event.target.value)) {
            set_Hook_level_3_Form(Hook_level_3.filter((checkedValue: any) => checkedValue !== event.target.value));
            // console.log('[Include]チェックボックスChecked個数', level_1);
        } else {
            Hook_level_3.push(event.target.value);
            set_Hook_level_3_Form(Hook_level_3);
        ...
        props.handleValueChange1(Hook_level_3);
    ...
    useEffect(() => {
        ...
        const usersCollectionRef1 = doc(db, 'front_hook', 'level_3');
...
export default Firebase_hook_level3;