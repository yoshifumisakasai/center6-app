...
const Firebase_framework_level1 = (props: any) => {

    let navigate = useNavigate();
    let goBack = () => {
        navigate(-1);
    };
    const [Framework_level_1, set_Framework_level_1_Form] = useState<any>([]);
    ...
    const check_level_1 = (event: any) => {
        ...
        //下記値は、インデックス番号
        if (Framework_level_1.includes(event.target.value)) {
            set_Framework_level_1_Form(Framework_level_1.filter((checkedValue: any) => checkedValue !== event.target.value));
            // console.log('[Include]チェックボックスChecked個数', level_1);
        } else {
            Framework_level_1.push(event.target.value);
            set_Framework_level_1_Form(Framework_level_1);
        ...
        props.handleValueChange1(Framework_level_1);
    ...
    // };
    const entity: HomeEntity = {
        Framework_level_1: Framework_level_1
    };
    ...
    useEffect(() => {
        ...
        const usersCollectionRef1 = doc(db, 'front_framework', 'level_1');
...
export default Firebase_framework_level1;