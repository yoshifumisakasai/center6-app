...
const Firebase_state_level2 = (props: any) => {

    let navigate = useNavigate();
    let goBack = () => {
        navigate(-1);
    };
    const [state_level_2, set_State_level_2_Form] = useState<any>([]);
    ...
    const check_level_1 = (event: any) => {
        ...
        //下記値は、インデックス番号
        if (state_level_2.includes(event.target.value)) {
            set_State_level_2_Form(state_level_2.filter((checkedValue: any) => checkedValue !== event.target.value));
            // console.log('[Include]チェックボックスChecked個数', level_1);
        } else {
            state_level_2.push(event.target.value);
            set_State_level_2_Form(state_level_2);
        ...
        props.handleValueChange1(state_level_2);
...
export default Firebase_state_level2;