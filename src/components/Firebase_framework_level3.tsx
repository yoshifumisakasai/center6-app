...
const Firebase_framework_level3 = (props: any) => {

    let navigate = useNavigate();
    let goBack = () => {
        navigate(-1);
    };
    const [Framework_level_3, set_Framework_level_3_Form] = useState<any>([]);
    ...
    const check_level_1 = (event: any) => {
        ...
        //下記値は、インデックス番号
        if (Framework_level_3.includes(event.target.value)) {
            set_Framework_level_3_Form(Framework_level_3.filter((checkedValue: any) => checkedValue !== event.target.value));
            // console.log('[Include]チェックボックスChecked個数', level_1);
        } else {
            Framework_level_3.push(event.target.value);
            set_Framework_level_3_Form(Framework_level_3);
        ...
        props.handleValueChange1(Framework_level_3);
    ...
    // };
    const entity: HomeEntity = {
        Framework_level_3: Framework_level_3
    };
    ...
    useEffect(() => {
        ...
        const usersCollectionRef1 = doc(db, 'front_framework', 'level_3');
...
export default Firebase_framework_level3;