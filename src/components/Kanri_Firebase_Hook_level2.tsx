...
function Kanri_Firebase_Hook_level2() {
    ...
    const handleSubmit = async (event: any) => {
        ...
        const userDocumentRef1 = doc(db, 'front_hook', 'level_2');
        if (input1 === "") {
        } else {
    ...
    const changeUser = async (name: any) => {
        ...
        const ref = doc(db, 'front_hook', selectedOption);
        if (level_1 === "") {
        } else {
            for (let i = 0; i < level_1.length; i++) {
                await updateDoc(ref,
...
export default Kanri_Firebase_Hook_level2;