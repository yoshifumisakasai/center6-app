...
const Firebase_css_level2 = (props: any) => {
    ...
    const check_level_1 = (event: any) => {
        ...
        //下記値は、インデックス番号
        if (css_level_2.includes(event.target.value)) {
            set_css_level_2_Form(css_level_2.filter((checkedValue: any) => checkedValue !== event.target.value));
            // console.log('[Include]チェックボックスChecked個数', level_1);
        } else {
            css_level_2.push(event.target.value);
            set_css_level_2_Form(css_level_2);
        ...
        props.handleValueChange1(css_level_2);
    ...
    useEffect(() => {
        ...
        const usersCollectionRef1 = doc(db, 'front_library', 'level_2');
...
export default Firebase_css_level2;