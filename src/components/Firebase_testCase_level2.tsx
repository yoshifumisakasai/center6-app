...
const Firebase_testCase_level2 = (props: any) => {

    let navigate = useNavigate();
    let goBack = () => {
        navigate(-1);
    };
    const [testCase_level_2, set_testCase_level_2_Form] = useState<any>([]);
    ...
    const check_level_1 = (event: any) => {
        ...
        //下記値は、インデックス番号
        if (testCase_level_2.includes(event.target.value)) {
            set_testCase_level_2_Form(testCase_level_2.filter((checkedValue: any) => checkedValue !== event.target.value));
            // console.log('[Include]チェックボックスChecked個数', level_1);
        } else {
            testCase_level_2.push(event.target.value);
            set_testCase_level_2_Form(testCase_level_2);
        ...
        props.handleValueChange1(testCase_level_2);
    ...
    // };
    const entity: HomeEntity = {
        testCase_level_2: testCase_level_2
    };
    ...
    useEffect(() => {
        ...
        const usersCollectionRef1 = doc(db, 'front_testCase', 'level_2');
...
export default Firebase_testCase_level2;